...
class Graphormer(nn.Module):
    def __init__(self, 
        ...
        self.read_out = nn.Linear(self.node_num,1)
    ...
    def forward(self, data: Union[Data]) -> torch.Tensor:
        ...
        # x = data[:0].float()
        # edge_index = data[:1].long()
        # edge_attr = data[:2].float()

        # 如果数据data是Data类，表示处理单个图数据
        if type(data) == Data:
            ptr = [0]
            node_paths, edge_paths = shortest_path_distance(data)
        else:
            ptr = data.ptr  # 如果不是，那么说明是批处理，则需要批量处理图的最短路径
        ...
        x = self.node_out_lin(x) # 得到每个节点的分类

        output = self.read_out(x.T) # readout出一张图的分类
