...
def tag_atoms_in_repeating_unit(mol):
    ...
    如果某原子为非连接点,则设core属性为Core,    将连接点原子的R属性设为编号
    """
    ...
    * :code:`overwrite_default_bond_features`: A boolean to overwrite default bond descriptors.
    ...
    def __init__(self, mol: Union[str, Chem.Mol, Tuple[Chem.Mol, Chem.Mol]],
        ...
        if not self.is_reaction and self.is_polymer:
            ...
            # 得到各个单体的链接关系以及聚合度（没有就是1）
            # 如：[('1', '3', 0.25, 0.25), ('1', '4', 0.25, 0.25), ('2', '3', 0.25, 0.25), ('2', '4', 0.25, 0.25), ('1', '2', 0.25, 0.25), ('3', '4', 0.25, 0.25), ('1', '1', 0.25, 0.25), ('2', '2', 0.25, 0.25), ('3', '3', 0.25, 0.25), ('4', '4', 0.25, 0.25)]
            ...
            self.f_atoms = [atom_features(atom) for atom in rwmol.GetAtoms() if atom.GetBoolProp('core') is True]           # 非连接点
            self.w_atoms = [atom.GetDoubleProp('w_frag') for atom in rwmol.GetAtoms() if atom.GetBoolProp('core') is True]  # 非连接点的权重
            ...
            rwmol = remove_wildcard_atoms(rwmol)    # 移除连接点
...
mol2graph(["[*:1]c1cc(F)c([*:2])cc1F.[*:3]c1cc([*:4])c(C(=O)OC)nc1N|0.5|0.5|<1-3:0.25:0.25<1-4:0.25:0.25<2-3:0.25:0.25<2-4:0.25:0.25<1-2:0.25:0.25<3-4:0.25:0.25<1-1:0.25:0.25<2-2:0.25:0.25<3-3:0.25:0.25<4-4:0.25:0.25"])